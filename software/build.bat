...
if not exist "JUCE\extras\Projucer\Builds\VisualStudio2022\x64\Release\App\Projucer.exe" (
	msbuild JUCE\extras\Projucer\Builds\VisualStudio2022\Projucer.sln /p:Configuration=Release /p:Platform=x64 /m
)


rmdir /s /q Builds
rmdir /s /q JuceLibraryCode
